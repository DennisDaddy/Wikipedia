...
class ArticleDataProvider {
    ...
    fun search(term: String, skip: Int, take: Int, respondHandler: (result: WikiResult) -> Unit?){
        Urls.getSearchUrl(term, skip, take).httpGet().responseObject(WikipediaDataDeserializer()){ _, response, result ->
          ...
          val(data, _) = result
            respondHandler.invoke(data as WikiResult)
        }
    }

    fun getRandom(take: Int, respondHandler: (result: WikiResult) -> Unit?){
